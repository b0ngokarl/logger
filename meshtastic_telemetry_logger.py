...
def collect_telemetry_cli(dest: str, serial_dev: Optional[str]=None, timeout: int=30) -> Optional[Dict[str, float]]:
    ...
    nodes_data = {}
    nodes_last_heard = None
    
    # Use the --nodes command to get a table of all nodes
    ...
    if ok:
        ...
        print(f"[DEBUG] Looking for node {dest} in nodes table output")
        
        # Parse the table output to find the specified node
        ...
        for line in lines:
            ...
            if dest in line:
                print(f"[DEBUG] Found node line: {line}")
                found = True
                
                # Split the line by the table separator │
                parts = line.split('│')
                if len(parts) >= 17:  # Make sure we have enough columns including LastHeard
                    try:
                        # Extract all node information from the table
                        # Column mapping based on the observed table format
                        nodes_data["user"] = parts[1].strip() if len(parts) > 1 else "Unknown"
                        nodes_data["id"] = parts[2].strip() if len(parts) > 2 else dest
                        nodes_data["aka"] = parts[3].strip() if len(parts) > 3 else ""
                        nodes_data["hardware"] = parts[4].strip() if len(parts) > 4 else ""
                        nodes_data["key"] = parts[5].strip() if len(parts) > 5 else ""
                        nodes_data["firmware"] = parts[6].strip() if len(parts) > 6 else ""
                        nodes_data["latitude"] = parts[7].strip() if len(parts) > 7 else ""
                        nodes_data["longitude"] = parts[8].strip() if len(parts) > 8 else ""
                        nodes_data["altitude"] = parts[9].strip() if len(parts) > 9 else ""
                        
                        # Extract last heard timestamp (column 17)
                        nodes_last_heard = parts[16].strip() if len(parts) > 16 else ""
                        nodes_data["last_seen"] = nodes_last_heard
                        
                        # Extract signal strength (column 14)
                        signal_str = parts[13].strip() if len(parts) > 13 else ""
                        if signal_str and signal_str != 'N/A':
                            if 'dB' in signal_str:
                                nodes_data["signal_strength"] = signal_str
                        
                        # Extract hop count (column 15)
                        hop_str = parts[14].strip() if len(parts) > 14 else ""
                        if hop_str and hop_str != 'N/A':
                            nodes_data["hops"] = hop_str
                        
                        # Extract battery data - column 11
                        battery_str = parts[10].strip() if len(parts) > 10 else ""
                        if battery_str and battery_str != 'N/A':
                            if '%' in battery_str:
                                nodes_data["battery_pct"] = float(battery_str.replace('%', ''))
                            elif battery_str == "Powered":
                                # For powered nodes, set to 100%
                                nodes_data["battery_pct"] = 100.0
                            print(f"[DEBUG] Battery: {battery_str} -> {nodes_data.get('battery_pct')}")
                        
                        # Extract channel utilization - column 12
                        chan_str = parts[11].strip() if len(parts) > 11 else ""
                        if chan_str and chan_str != 'N/A' and '%' in chan_str:
                            nodes_data["channel_util_pct"] = float(chan_str.replace('%', ''))
                            print(f"[DEBUG] Channel utilization: {chan_str} -> {nodes_data.get('channel_util_pct')}")
                        
                        # Extract air transmission - column 13
                        air_str = parts[12].strip() if len(parts) > 12 else ""
                        if air_str and air_str != 'N/A' and '%' in air_str:
                            nodes_data["air_tx_pct"] = float(air_str.replace('%', ''))
                            print(f"[DEBUG] Air transmission: {air_str} -> {nodes_data.get('air_tx_pct')}")
                        
                        print(f"[DEBUG] Extracted node data from table: {nodes_data}")
                    except (ValueError, IndexError) as e:
                        print(f"[DEBUG] Error parsing node data: {e}")
                break
        
        if not found:
            print(f"[DEBUG] Node {dest} not found in nodes table")
    else:
        print(f"[WARN] Telemetry CLI nodes command failed for {dest}:\n{out}", file=sys.stderr)
    
    # Now try to request fresh telemetry data using --request-telemetry
    print(f"[DEBUG] Requesting fresh telemetry data for {dest}")
    request_cmd = ["meshtastic", "--request-telemetry", "--dest", dest]
    if serial_dev:
        request_cmd += ["--port", serial_dev]
    
    request_data = {}
    request_last_heard = None
    
    request_ok, request_out = run_cli(request_cmd, timeout=timeout*2)  # Double timeout for telemetry request
    
    if request_ok:
        print(f"[DEBUG] Got telemetry request response: {request_out}")
        
        # Parse the output for telemetry data
        # Look for timestamp in response
        timestamp_match = re.search(r"Received telemetry at: (.+)", request_out)
        if timestamp_match:
            request_last_heard = timestamp_match.group(1)
        
        # Parse for specific telemetry metrics
        batt_match = RE_BATT.search(request_out)
        if batt_match:
            request_data["battery_pct"] = float(batt_match.group(1))
        
        volt_match = RE_VOLT.search(request_out)
        if volt_match:
            request_data["voltage_v"] = float(volt_match.group(1))
        
        chan_match = RE_CHAN.search(request_out)
        if chan_match:
            request_data["channel_util_pct"] = float(chan_match.group(1))
        
        air_match = RE_AIR.search(request_out)
        if air_match:
            request_data["air_tx_pct"] = float(air_match.group(1))
        
        up_match = RE_UP.search(request_out)
        if up_match:
            request_data["uptime_s"] = float(up_match.group(1))
            
        print(f"[DEBUG] Extracted metrics from telemetry request: {request_data}")
    else:
        print(f"[DEBUG] Telemetry request failed or timed out: {request_out}")
    
    # Merge data based on which is more recent
    final_data = {}
    
    # More intelligent merging approach: merge by individual metrics
    print(f"[DEBUG] Performing intelligent metric merging between nodes table and telemetry request")
    final_data = {}
    
    # Define which data source is considered more recent for which metrics
    nodes_table_is_fresher = bool(not request_last_heard or
                                (nodes_last_heard and nodes_last_heard > request_last_heard))
    
    # These metrics are generally more accurate from nodes table
    nodes_table_preferred_metrics = ["channel_util_pct", "air_tx_pct"]
    # These metrics are generally more accurate from telemetry request
    request_preferred_metrics = ["battery_pct", "voltage_v", "uptime_s"]
    
    # First, add all metrics from both sources
    if request_data:
        for key, value in request_data.items():
            # For metrics that are better from request, always use request data
            if key in request_preferred_metrics:
                final_data[key] = value
                print(f"[DEBUG] Using telemetry request data for {key}={value} (preferred source)")
            else:
                # For other metrics, check timestamp
                if not nodes_data or key not in nodes_data:
                    final_data[key] = value
                    print(f"[DEBUG] Using telemetry request data for {key}={value} (only source)")
    
    if nodes_data:
        for key, value in nodes_data.items():
            # For metrics that are better from nodes table, always use nodes data
            if key in nodes_table_preferred_metrics:
                final_data[key] = value
                print(f"[DEBUG] Using nodes table data for {key}={value} (preferred source)")
            elif key not in final_data:
                # For other metrics not yet added, use nodes data
                final_data[key] = value
                print(f"[DEBUG] Using nodes table data for {key}={value} (only source)")
    
    # Summary of data source used
    if nodes_data and request_data:
        print(f"[DEBUG] Merged data from both sources, prioritizing by metric type")
    elif nodes_data:
        print(f"[DEBUG] Using nodes table data only (no telemetry request data)")
    elif request_data:
        print(f"[DEBUG] Using telemetry request data only (no nodes table data)")
    
    # If we still don't have data, fall back to --info command
    if not final_data:
        print(f"[DEBUG] Falling back to --info command for {dest}")
        cmd = ["meshtastic", "--info"]
        if serial_dev:
            cmd += ["--port", serial_dev]
        ok, out = run_cli(cmd, timeout=timeout)
        
        # Parse the output using our regex patterns for better accuracy
        batt_match = RE_BATT.search(out)
        if batt_match:
            final_data["battery_pct"] = float(batt_match.group(1))
        
        volt_match = RE_VOLT.search(out)
        if volt_match:
            final_data["voltage_v"] = float(volt_match.group(1))
        
        chan_match = RE_CHAN.search(out)
        if chan_match:
            final_data["channel_util_pct"] = float(chan_match.group(1))
        
        air_match = RE_AIR.search(out)
        if air_match:
            final_data["air_tx_pct"] = float(air_match.group(1))
        
        up_match = RE_UP.search(out)
        if up_match:
            final_data["uptime_s"] = float(up_match.group(1))
    
    # Debug what we found
    print(f"[DEBUG] Final telemetry data for {dest}: {final_data}")
            ...
            if len(parts) > 1:
                try:
                    node = {
                        ...
                        "id": parts[3].strip(),
                        "aka": parts[4].strip(),
                        "latitude": parts[8].strip(),
                        "longitude": parts[9].strip(),
                        "last_seen": parts[17].strip(),
                        "hops": parts[15].strip(),
                    }
                    nodes.append(node)
                except IndexError:
